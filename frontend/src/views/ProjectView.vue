...
<template>
  <section class="feed-container w-full my-8 px-20 md:px-28 lg:px-44 lg:mt-20">
    ...
    <p class="text-slate-600 mb-6 dark:text-slate-100">A selection of my recent projects hosted on GitHub. Click "Code" to view the source code.</p>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div v-for="repo in repos" :key="repo.id" 
        ...
        <div class="flex justify-between items-start mb-4">
          <h3 class="font-bold text-lg group-hover:text-blue-600 truncate select-none">{{ repo.name }}</h3>
          <span class="text-xs font-medium px-2 py-1 bg-slate-100 rounded-full select-none">{{ repo.language }}</span>
        </div>

        <p class="text-slate-600 text-sm line-clamp-2 mb-4 select-none">
          {{ repo.description }}
        </p>
